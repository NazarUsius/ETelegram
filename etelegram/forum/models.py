...
class LikeBranch(models.Model):
    ...
    def __str__(self):
        return f"Like by {self.user.username} on post: {self.branch.title}"


class DislikeBranch(models.Model):
    branch = models.ForeignKey(Branch, related_name="dislikes", on_delete=models.CASCADE)
    user = models.ForeignKey(settings.AUTH_USER_MODEL, related_name="disliked_posts", on_delete=models.CASCADE)
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"Dislike by {self.user.username} on post: {self.branch.title}"
...
class Comment(models.Model):
    ...
    def __str__(self):
        return f"Comment by {self.user.username} on post {self.branch.title}"


class LikeComment(models.Model):
    comment = models.ForeignKey(Comment, related_name="likes", on_delete=models.CASCADE)
    user = models.ForeignKey(settings.AUTH_USER_MODEL, related_name="liked_comments", on_delete=models.CASCADE)
    created_at = models.DateTimeField(auto_now_add=True)
...
class DislikeComment(models.Model):
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL, related_name="disliked_comments", on_delete=models.CASCADE)
    created_at = models.DateTimeField(auto_now_add=True)
